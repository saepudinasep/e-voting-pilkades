...
export default function Index({ elections }) {
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold">Kelola Pemilihan</h2>}>
                        ...
                        <tbody>
                            {elections.data.map((el) => (
                                <tr key={el.id} className="border-t">
                                    <td className="px-4 py-3 font-medium">{el.nama}</td>
                                    <td className="px-4 py-3">{el.wilayah}</td>
                                    <td className="px-4 py-3">
                                        <span className={`px-2 py-1 rounded text-xs ${statusBadge[el.status]}`}>
                                            {el.status}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3">
                                        <Link
                                            href={route('admin.elections.positions.index', el.id)}
                                            className="text-blue-600 hover:underline"
                                        >
                                            {el.positions_count} posisi
                                        </Link>
                                    </td>
                                    <td className="px-4 py-3">{el.voters_count}</td>
                                    <td className="px-4 py-3">
                                        <Link
                                            href={route('admin.elections.tps.index', el.id)}
                                            className="text-blue-600 hover:underline"
                                        >
                                    ...
                                    <td className="px-4 py-3 text-right space-x-2">
                                        <Link
                                            href={route('admin.elections.edit', el.id)}
